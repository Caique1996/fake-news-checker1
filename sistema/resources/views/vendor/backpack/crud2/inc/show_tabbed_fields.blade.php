...
<div class="tab-container {{ $horizontalTabs ? '' : 'container'}} mb-2">
    ...
    <div class="nav-tabs-custom {{ $horizontalTabs ? '' : 'row'}}" id="form_tabs">
        ...
        <div class="tab-content p-0 {{$horizontalTabs ? '' : 'col-md-9'}}">
            ...
            @foreach ($crud->getTabs() as $k => $tab)
                <div role="tabpanel"
                     class="tab-pane {{ isset($tabWithError) && $tabWithError ? ($tab == $tabWithError ? ' active' : '') : ($k == 0 ? ' active' : '') }}"
                     id="tab_{{ Str::slug($tab) }}">
                        <?php
                        $fieldsTab = $crud->getTabFields($tab)->toArray();
                        ?>
                    <div class="row">
                        @include('crud::inc.show_fields', ['fields' => addLabelToFields($fieldsTab)])
                    </div>
                </div>
            @endforeach
